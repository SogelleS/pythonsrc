import os, re


def application(requ_data, new_client_addr, daf_root_path):
    requ_path = parsepath(requ_data,daf_root_path)
    print('请求ip[%s]目标路径:[%s]' % (new_client_addr[0], requ_path))

    if os.path.isfile(requ_path):
        status = 200
        with open(requ_path, 'rb') as fp:
            resp_data = fp.read()

    else:
        status = 404
        resp_data = 'can\'t find page'.encode('utf-8')

    resp_total = creatRespTelegram(resp_data, status)

    return resp_total


def parsepath(requ_data,daf_root_path):
    root_path = 'E:\\pythonsrc\\python进阶\\测试网页'

    requ_path = re.search('(GET|HEAD) (.*) HTTP', requ_data.decode('utf-8'))
    try:
        not_root_path = re.sub('/', '\\\\', requ_path.group(2))
    except Exception as e:
        print(requ_data)
        exit(1)
    # b'HEAD / HTTP/1.1\r\nAccept: */*\r\nHost:3049r005m8.qicp.vip\r\n\r\n'

    requ_path = root_path + re.sub('/', '\\\\', requ_path.group(2))

    if requ_path == 'E:\\pythonsrc\\python进阶\\测试网页\\':
        requ_path = 'E:\\pythonsrc\\python进阶\\测试网页\\%s\\index.html' % daf_root_path
        return requ_path

    else:
        requ_path = 'E:\\pythonsrc\\python进阶\\测试网页\\%s%s' % (daf_root_path,not_root_path)
        return requ_path


# 可以将creatRespTelegram函数封装到另一个.py文件中

def creatRespTelegram(resp_body, status):
    resp_line = 'HTTP/1.1 %d OK\r\n' % status
    resp_hesder = 'Server:pyweb\r\n'
    resp_blank = '\r\n'
    return (resp_line + resp_hesder + resp_blank).encode('utf-8') + resp_body



